...
<template>
    <div class="w-[90%] fle flex-col justify-center">

        <div v-if="restaurant" class="mb-6 flex flex-col bg-white rounded-xl shadow-lg p-6">
            <h1 class="text-3xl font-bold text-[#03CAED] mb-2">Dashboard: {{ restaurant.naam }}</h1>
            <div class="flex gap-6 max-[768px]:flex-col text-gray-600">
        ...
        <div class="mb-4">
            <button @click="router.push(`/restaurant/${restaurantId}`)"
                class="px-4 py-2 bg-[#03CAED] text-white rounded-lg hover:bg-[#02a8c4] transition">
                ← Terug naar restaurant pagina
            </button>
        </div>

        <div v-if="isLoading" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-999">
        ...
        <div class="relative w-full max-[768px]:overflow-x-auto max-[768px]:overflow-y-hidden rounded-lg"
            style="min-height: 1240px;">
            <div class="max-[768px]:min-w-[1200px]">
                <div v-for="(t, i) in timeLabels" :key="i" class="absolute w-full flex items-center"
                    :style="{ top: (i * 80) + 'px', left: '0', height: '80px', width: '100%' }">
                    <div class="w-[100px] font-jockey text-right pr-10 text-[28px] text-[#03CAED]">
                        {{ t }}
                    </div>
                    <div class="max-[768px]:min-w-[1200px]" style="flex:1; height: 3px; background: orange;"></div>
                </div>

                <div v-for="r in reservationsForSelectedDay" :key="r.id" :style="reservationStyle(r)"
                    @click="openDrawerFor(r)" class="hover:opacity-90 transition">
        ...
        <div v-if="showDrawer" class="fixed inset-0 z-176 flex" aria-hidden="false">
            ...
            <aside
                class="ml-auto bg-white shadow-xl h-full w-[620px] p-6 transform transition-transform relative z-10 overflow-y-auto">
                ...
                <div class="grid grid-cols-4 gap-2 mb-4">
                    <button v-for="t in timeSlots" :key="t" @click="!isTimeFullForEdit(t) && (editTime = t)"
                        class="w-[75px] p-2 border rounded text-sm transition" :class="{
                ...
                <label class="block mb-4">
                    ...
                    <input v-model="editPeople" type="number" min="1" :max="capacity"
                        class="border p-2 w-full mt-1 rounded" />
                </label>

                <div class="flex justify-between mt-6 gap-4">
                    <button @click="deleteReservation"
                        ...
                        <div v-else
                            class="w-5 h-5 border-4 border-t-transparent border-white rounded-full animate-spin"></div>
                    </button>
